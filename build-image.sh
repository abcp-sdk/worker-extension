#!/usr/bin/env bash
# Build and push the worker-extension image WITHOUT a local build daemon.
# Mirrors workspace-extension/build-image.sh: buildkitd -> docker archive ->
# skopeo -> forgejo OCI.
set -euo pipefail
