...
export interface FileReadState {
  ...
  updatedAt: number
  /**
   * Windowed-read metadata (0-based, end-exclusive): the sha256 covers
   * lines [winStart, winEnd) fetched from the worker. `edit` re-fetches the
   * SAME window and compares hashes — as sound as a whole-file hash for
