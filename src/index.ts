...
const CONFIG_SPECS: Record<string, ConfigSpec> = {
  [CONFIG.workerUrl]: {
    ...
    scope: 'global',
    description:
      'Base URL of the easyworker sandbox (e.g. http://127.0.0.1:9090).',
    descriptions: {
      zh: 'easyworker 沙箱的基础地址（如 http://127.0.0.1:9090）。',
    },
  },
  [CONFIG.workerToken]: {
    type: 'string',
    ...
    scope: 'global',
    description:
      'easyworker bearer token (empty only when the worker has auth disabled).',
    descriptions: {
      zh: 'easyworker 的 bearer 令牌（仅当 worker 关闭鉴权时才可留空）。',
    },
  },
}
...
const str = (
  ...
  type: 'string',
  description,
  ...(descriptionsZh !== undefined
    ? { descriptions: { zh: descriptionsZh } }
    : {}),
})
...
/**
 * Appended to every command-running tool (`exec` / `job-start`) and to their
 * `command` argument. The remote worker already runs each job as a tracked
 * background task and records its output, so shell-level backgrounding and
 * output redirection actively break observability.
 */
...
const TOOL_META: Record<string, ToolMeta> = {
  info: {
    description:
      'Show the connected easyworker environment: OS/arch, shell, workspace root, service DNS and boot id.',
    descriptions: {
      zh: '查看所连 easyworker 的环境：OS/架构、shell、工作区根目录、服务域名与 boot id。',
    },
    inputSchema: obj({}),
  },
  exec: {
    ...
    descriptions: {
      zh:
        '在工作区运行一个短命令，最多等待 `timeout` 秒。总是返回 job id；完成时最多返回 1000 行输出，超时则返回最旧的 200 行并提示任务仍在运行。' +
        EXEC_WARNING_ZH,
    },
    inputSchema: obj(
      {
        command: str(
          `Shell command to run. ${EXEC_WARNING_EN}`,
          `要运行的 shell 命令。${EXEC_WARNING_ZH}`,
        ),
        workdir: str(
        ...
        timeout: int(
          'Synchronous wait ceiling in seconds (default 5, max 60).',
          '同步等待上限（秒，默认 5，最大 60）。',
        ),
        env: {
          type: 'object',
  ...
  'job-start': {
    description:
      'Start a long-running shell command and return the job id immediately (no waiting). Drive it with job-wait/job-output/job-stdin/job-kill. ' +
      EXEC_WARNING_EN,
    descriptions: {
      zh:
        '启动一个长时间运行的 shell 命令并立即返回 job id（不等待）。用 job-wait/job-output/job-stdin/job-kill 进行后续控制。' +
        EXEC_WARNING_ZH,
    },
    inputSchema: obj(
      {
  ...
  'job-output': {
    description:
      "Read a job's output by line window (offset/limit; offset negative counts from the end). Display is capped at 1000 lines / 120 KiB.",
    descriptions: {
      zh: '按行窗口读取任务输出（offset/limit；offset 为负从末尾计数）。展示上限 1000 行 / 120 KiB。',
    },
    inputSchema: obj(
      {
        'job-id': JOB_ID,
        offset: int(
          'Start line offset (negative = from the end, default 0).',
          '起始行偏移（负值从末尾算，默认 0）。',
        ),
        limit: int(
          'Maximum lines to return (default 200, max 1000).',
          '最多返回行数（默认 200，最大 1000）。',
        ),
        stream: {
          type: 'string',
  ...
  'job-wait': {
    description:
      'Wait up to `timeout` seconds for a job to finish; returns the latest 200 lines of output. If it is still running at the deadline, says so.',
    descriptions: {
      zh: '最多等待 `timeout` 秒让任务结束；返回最新的 200 行输出。若到点仍在运行则明确提示。',
    },
    inputSchema: obj(
      {
        'job-id': JOB_ID,
        timeout: int(
          'Wait ceiling in seconds (default 60, max 600).',
          '等待上限（秒，默认 60，最大 600）。',
        ),
      },
      ['job-id'],
  ...
  'job-list': {
    description:
      'List jobs registered in the worker (id, state, exit code, command).',
    descriptions: {
      zh: '列出 worker 中登记的任务（id、状态、退出码、命令）。',
    },
    inputSchema: obj({}),
  },
  read: {
    description:
      'Read a text file from the workspace with line numbers, windowed by offset/limit. Binary files are rejected. Reading records the lines as "seen" so a later edit may change them.',
    descriptions: {
      zh: '从工作区读取文本文件并带行号，通过 offset/limit 分窗。二进制文件会被拒绝。读取会记录已“看到”的行，之后 edit 才能修改这些行。',
    },
    inputSchema: obj(
      {
        path: str(
        ...
        offset: int(
          'Start line (0-based, default 0).',
          '起始行（从 0 开始，默认 0）。',
        ),
        limit: int(
          'Maximum lines (default 200, max 1000).',
          '最多行数（默认 200，最大 1000）。',
        ),
      },
      ['path'],
  ...
  write: {
    description:
      'Write (overwrite) a text file in the workspace, then return the whole file with line numbers. Rejected if the content exceeds 120 KiB. The whole file counts as "seen", so it can be edited afterwards.',
    descriptions: {
      zh: '向工作区写入（覆盖）文本文件，随后返回带行号的全文件。内容超过 120 KiB 会被拒绝。整个文件视为已“看到”，之后可直接 edit。',
    },
    inputSchema: obj(
      {
        path: str(
  ...
  edit: {
    description:
      'Edit a workspace file by line numbers (1-based). When end-line < start-line it inserts before start-line; otherwise it replaces [start-line, end-line]. Out-of-range line numbers are clamped to the file. The session must have read (or written) the file first, and may only edit lines it has seen; the file must be unchanged since that read. A successful edit requires a fresh read before the next edit (line numbers may shift). Returns a one-line summary followed by a unified diff.',
    descriptions: {
      zh: '按行号（从 1 开始）编辑工作区文件。end-line < start-line 时在 start-line 前插入；否则替换 [start-line, end-line]。越界行号会夹取到文件范围。会话必须先 read（或 write）过该文件，且只能修改已“看到”的行；文件自上次读取后不得变化。编辑成功后需重新 read 才能再次 edit（行号可能已改变）。返回一行摘要及本次改动的 unified diff。',
    },
    inputSchema: obj(
      {
        path: str(
        ...
        'end-line': int(
          'End line (1-based, inclusive); < start-line means insert.',
          '结束行（从 1 开始，含端点）；小于 start-line 表示插入。',
        ),
  ...
  list: {
    description:
      'List a workspace path as a breadth-first tree (levels 1..depth) with sizes. Directories beyond the cap stay collapsed.',
    descriptions: {
      zh: '以广度优先树（第 1..depth 层）列出工作区路径并显示大小。超过上限的目录保持折叠。',
    },
    inputSchema: obj(
      {
        path: str(
        ...
        limit: int(
          'Maximum entries (default 200, max 1000).',
          '最大条目数（默认 200，最大 1000）。',
        ),
  ...
  download: {
    description:
      'Download a stored file (agent `file:<code>`) into the workspace at `path`.',
    descriptions: {
      zh: '将已存储文件（agent 的 `file:<code>`）下载到工作区的 `path`。',
    },
    inputSchema: obj(
      {
        code: str(
          'File code (with or without the `file:` prefix).',
          '文件 code（可带或不带 `file:` 前缀）。',
        ),
  ...
  upload: {
    description:
      'Upload a workspace file to the agent file store, returning its `file:<code>`. The content type is derived by the agent.',
    descriptions: {
      zh: '将工作区文件上传到 agent 文件存储，返回 `file:<code>`。内容类型由 agent 推断。',
    },
    inputSchema: obj(
      {
        path: str(
        ...
        name: str(
          'Stored file name (defaults to the basename).',
          '存储文件名（默认取 basename）。',
        ),
      },
      ['path'],
