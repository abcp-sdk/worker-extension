...
async function main(): Promise<void> {
  ...
  const { stop } = await serveWorker({ natsUrl })
  console.log(
    '[worker] serving over %s; configure worker-url / worker-token in tool settings',
    natsUrl,
  )
  ...
  const http = createServer((req, res) => {
    ...
    if (req.url === '/api/v1/health') {
      res.end(JSON.stringify({ ok: true, id: 'worker' }))
      return
    }
    res.statusCode = 404
    res.end('{}')
  })
  http.listen(port, () => console.log(`[worker] http :${port}`))
